...
class EGIProfileTest(unittest.TestCase):
        ...
        if "GLUE2EntityCreationTime" not in self.entry:
            ...
            try:
                creationtime = datetime.datetime(
                    *(
                        time.strptime(
                            self.entry["GLUE2EntityCreationTime"][0],
                            "%Y-%m-%dT%H:%M:%SZ",
                        )[0:6]
                    )
                )
    ...
    def test_GLUE2EntityOtherInfo_OK(self):
        message = (
            "%s contains the following issues in the GLUE2EntityOtherInfo attribute"
            % (self.dn)
        )
        sharedict = {}
        status = True
                ...
                if att == "ProfileVersion":
                    ...
                    if not self.types.is_Tier_t(val):
                        message = message + validator.utils.message_generator(
                            ...
                            "I006",
                            self.dn,
                            "GLUE2EntityOtherInfo: WLCG_TIER",
                            val,
                        )
                        status = False
                ...
                #        status = False
                    ...
                    if not self.types.is_URL(val):
                        message = message + validator.utils.message_generator(
                            ...
                            "I008",
                            self.dn,
                            "GLUE2EntityOtherInfo: WLCG_NAMEICON",
                            val,
                        )
                        status = False
                    ...
                    if not self.types.is_Middleware_t(val):
                        message = message + validator.utils.message_generator(
                            ...
                            "I009",
                            self.dn,
                            "GLUE2EntityOtherInfo: MiddlewareName",
                            val,
                        )
                        status = False
                    ...
                    if not self.types.is_String(val):
                        message = message + validator.utils.message_generator(
                            ...
                            "I010",
                            self.dn,
                            "GLUE2EntityOtherInfo: MiddlewareVersion",
                            val,
                        )
                        status = False
                elif att == "HostDN":
                    ...
                    if not self.types.is_Benchmarkabbr_t(
                        ...
                        message = message + validator.utils.message_generator(
                            ...
                            "I013",
                            self.dn,
                            "GLUE2EntityOtherInfo: CPUScalingReference",
                            att.split("CPUScalingReference")[1],
                        )
                        status = False
        totalshare = 0
    ...
    def test_GLUE2ServiceAdminDomainForeignKey_OK(self):
        ...
        if (
            self.entry["GLUE2ServiceType"][0] != "SRM"
        ):  # StoRM services are published as 'SRM' type for versions < 1.11.5
            substring = "GLUE2DomainID=.*,"
            m = re.search(substring, self.dn)
            domain, _ = m.group(0).split(",", 1)
            _, domain_name = domain.split("=", 1)
    ...
    def test_GLUE2LocationLongitude_OK(self):
        ...
        self.assertTrue(
            float(self.value[0]) > -180 and float(self.value[0]) < 180, message
        )
    ...
    def test_GLUE2ComputingServiceTotalJobs_OK(self):
        ...
        job_stats = ""
        for job in [
            "GLUE2ComputingServiceRunningJobs",
        ...
        # high = high + 1
        job_stats = job_stats + "; %s <= %s <= %s; Difference is %s" % (
            low,
            total,
            high,
        ...
        message = validator.utils.message_generator(
            ...
            "W005",
            self.dn,
            "GLUE2ComputingServiceTotalJobs",
            self.value[0],
            job_stats,
        )
        ...
        try:
            ...
            if "GLUE2EndpointDowntimeEnd" in self.entry:
                endtime = datetime.datetime(
                    *(
                        time.strptime(
                            self.entry["GLUE2EndpointDowntimeEnd"][0],
                            "%Y-%m-%dT%H:%M:%SZ",
                        )[0:6]
                    )
                )
    ...
    def test_GLUE2ComputingShareMinWallTime_OK(self):
        if "GLUE2ComputingShareMaxWallTime" in self.entry:
            message = validator.utils.message_generator(
                ...
                self.value[0],
                "GLUE2ComputingShareMaxWallTime is %s"
                % self.entry["GLUE2ComputingShareMaxWallTime"][0],
            )
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2ComputingShareMaxWallTime"][0]),
                message,
            )
    ...
    def test_GLUE2ComputingShareDefaultWallTime_MinRange(self):
        if "GLUE2ComputingShareMinWallTime" in self.entry:
            message = validator.utils.message_generator(
                ...
                "GLUE2ComputingShareDefaultWallTime",
                self.value[0],
                "GLUE2ComputingShareMinWallTime is %s"
                % self.entry["GLUE2ComputingShareMinWallTime"][0],
            )
            self.assertTrue(
                int(self.value[0])
                >= int(self.entry["GLUE2ComputingShareMinWallTime"][0]),
                message,
            )
    ...
    def test_GLUE2ComputingShareMinCPUTime_OK(self):
        if "GLUE2ComputingShareMaxCPUTime" in self.entry:
            message = validator.utils.message_generator(
                ...
                "W017",
                self.dn,
                "GLUE2ComputingShareMinCPUTime",
                self.value[0],
                "GLUE2ComputingShareMaxCPUTime is %s"
                % self.entry["GLUE2ComputingShareMaxCPUTime"][0],
            )
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2ComputingShareMaxCPUTime"][0]),
                message,
            )
    ...
    def test_GLUE2ComputingShareDefaultCPUTime_MinRange(self):
        if "GLUE2ComputingShareMinCPUTime" in self.entry:
            message = validator.utils.message_generator(
                ...
                "GLUE2ComputingShareDefaultCPUTime",
                self.value[0],
                "GLUE2ComputingShareMinCPUTime is %s"
                % self.entry["GLUE2ComputingShareMinCPUTime"][0],
            )
            self.assertTrue(
                int(self.value[0])
                >= int(self.entry["GLUE2ComputingShareMinCPUTime"][0]),
                message,
            )
    ...
    def test_GLUE2ComputingShareMaxTotalJobs_MaxRunningJobs(self):
        if "GLUE2ComputingShareMaxRunningJobs" in self.entry:
            message = validator.utils.message_generator(
                ...
                self.value[0],
                "GLUE2ComputingShareMaxRunningJobs is %s"
                % self.entry["GLUE2ComputingShareMaxRunningJobs"][0],
            )
            self.assertTrue(
                int(self.value[0])
                >= int(self.entry["GLUE2ComputingShareMaxRunningJobs"][0]),
                message,
            )
    ...
    def test_GLUE2ComputingShareMaxTotalJobs_MaxWaitingJobs(self):
        if "GLUE2ComputingShareMaxWaitingJobs" in self.entry:
            message = validator.utils.message_generator(
                ...
                "GLUE2ComputingShareMaxTotalJobs",
                self.value[0],
                "GLUE2ComputingShareMaxWaitingJobs is %s"
                % self.entry["GLUE2ComputingShareMaxWaitingJobs"][0],
            )
            self.assertTrue(
                int(self.value[0])
                >= int(self.entry["GLUE2ComputingShareMaxWaitingJobs"][0]),
                message,
            )
    ...
    def test_GLUE2ComputingShareMaxMainMemory_MinRange(self):
        message = validator.utils.message_generator(
            ...
            "W026",
            self.dn,
            "GLUE2ComputingShareMaxMainMemory",
            self.value[0],
        )
    ...
    def test_GLUE2ComputingShareGuaranteedMainMemory_MinRange(self):
        if "GLUE2ComputingShareMaxMainMemory" in self.entry:
            message = validator.utils.message_generator(
                ...
                self.value[0],
                "GLUE2ComputingShareMaxMainMemory is %s"
                % self.entry["GLUE2ComputingShareMaxMainMemory"][0],
            )
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2ComputingShareMaxMainMemory"][0]),
                message,
            )
    ...
    def test_GLUE2ComputingShareGuaranteedVirtualMemory_MinRange(self):
        if "GLUE2ComputingShareMaxVirtualMemory" in self.entry:
            ...
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2ComputingShareMaxVirtualMemory"][0]),
                message,
            )
    ...
    def test_GLUE2ComputingManagerTotalPhysicalCPUs_OK(self):
        if "GLUE2ComputingManagerTotalLogicalCPUs" in self.entry:
            message = validator.utils.message_generator(
                ...
                self.value[0],
                "GLUE2ComputingManagerTotalLogicalCPUs is %s"
                % self.entry["GLUE2ComputingManagerTotalLogicalCPUs"][0],
            )
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2ComputingManagerTotalLogicalCPUs"][0]),
                message,
            )
    ...
    def test_GLUE2ComputingManagerTotalSlots_OK(self):
        if "GLUE2ComputingManagerTotalLogicalCPUs" in self.entry:
            ...
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2ComputingManagerTotalLogicalCPUs"][0]) * 2,
                message,
            )
    ...
    def test_GLUE2ComputingManagerSlotsUsedByLocalJobs_OK(self):
        if "GLUE2ComputingManagerTotalSlots" in self.entry:
            message = validator.utils.message_generator(
                ...
                self.value[0],
                "GLUE2ComputingManagerTotalSlots is %s"
                % self.entry["GLUE2ComputingManagerTotalSlots"][0],
            )
            self.assertTrue(
                int(self.value[0])
                < int(self.entry["GLUE2ComputingManagerTotalSlots"][0]),
                message,
            )
    ...
    def test_GLUE2ExecutionEnvironmentUsedInstances_OK(self):
        if "GLUE2ExecutionEnvironmentTotalInstances" in self.entry:
            message = validator.utils.message_generator(
                ...
                self.value[0],
                "GLUE2ExecutionEnvironmentTotalInstances is %s"
                % self.entry["GLUE2ExecutionEnvironmentTotalInstances"][0],
            )
            self.assertTrue(
                int(self.value[0])
                < int(self.entry["GLUE2ExecutionEnvironmentTotalInstances"][0]),
                message,
            )
    ...
    def test_GLUE2ApplicationEnvironmentFreeSlots_OK(self):
        if "GLUE2ApplicationEnvironmentMaxSlots" in self.entry:
            message = validator.utils.message_generator(
                ...
                self.value[0],
                "GLUE2ApplicationEnvironmentMaxSlots is %s"
                % self.entry["GLUE2ApplicationEnvironmentMaxSlots"][0],
            )
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2ApplicationEnvironmentMaxSlots"][0]),
                message,
            )
    ...
    def test_GLUE2ApplicationEnvironmentFreeJobs_OK(self):
        if "GLUE2ApplicationEnvironmentMaxJobs" in self.entry:
            message = validator.utils.message_generator(
                ...
                self.value[0],
                "GLUE2ApplicationEnvironmentMaxJobs is %s"
                % self.entry["GLUE2ApplicationEnvironmentMaxJobs"][0],
            )
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2ApplicationEnvironmentMaxJobs"][0]),
                message,
            )
    ...
    def test_GLUE2ApplicationEnvironmentFreeUserSeats_OK(self):
        if "GLUE2ApplicationEnvironmentMaxUserSeats" in self.entry:
            message = validator.utils.message_generator(
                ...
                self.value[0],
                "GLUE2ApplicationEnvironmentMaxUserSeats is %s"
                % self.entry["GLUE2ApplicationEnvironmentMaxUserSeats"][0],
            )
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2ApplicationEnvironmentMaxUserSeats"][0]),
                message,
            )
    ...
    def test_GLUE2StorageServiceCapacityFreeSize_OK(self):
        if "GLUE2StorageServiceCapacityTotalSize" in self.entry:
            message = validator.utils.message_generator(
                ...
                "GLUE2StorageServiceCapacityFreeSize",
                self.value[0],
                "GLUE2StorageServiceCapacityTotalSize is %s"
                % self.entry["GLUE2StorageServiceCapacityTotalSize"][0],
            )
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2StorageServiceCapacityTotalSize"][0]),
                message,
            )
        ...
        for share in [
            ...
            if share in self.entry:
                if (share == "GLUE2StorageShareCapacityReservedSize") and (
                    self.entry["GLUE2StorageShareCapacityReservedSize"][0]
                    == self.value[0]
                ):
                    share_stats = share_stats + " %s=%s (Not added)" % (
                        share,
                        self.entry[share][0],
                    )
                else:
    ...
    def test_GLUE2StorageShareCapacityFreeSize_OK(self):
        if "GLUE2StorageShareCapacityTotalSize" in self.entry:
            message = validator.utils.message_generator(
                ...
                "GLUE2StorageShareCapacityFreeSize",
                self.value[0],
                "GLUE2StorageShareCapacityTotalSize is %s"
                % self.entry["GLUE2StorageShareCapacityTotalSize"][0],
            )
            self.assertTrue(
                int(self.value[0])
                <= int(self.entry["GLUE2StorageShareCapacityTotalSize"][0]),
                message,
            )
